...
wetlands %>% st_drop_geometry() %>% write_csv(., paste0(data_dir, "III_Products/wetlands_data.csv"))

#Create flowpath raster~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#breach
breach_depressions(dem = paste0(data_dir, "III_Products/dem_burn.tif"), 
...
#Select Catchments
ID<-c(894,886,897,924,933,936,941,952,972,959, 989, 965, 973, 1009, 1044, 643, 636, 655, 805, 833,832, 829, 932)
proposed_catchments<-wet_basins %>% filter(WetID %in% ID)
...
labels <- sprintf(
  "<strong>WetID = %s</strong>
   ...
   <br/>Mean Elevation = %g m
   <br/>HANS = %g m",
   wetlands$WetID, wetlands$watershed_area_m2/10000, wetlands$wet_order, wetlands$p_a_ratio, 
   wetlands$wetland_hsc_cm, wetlands$watershed_hsc_cm, wetlands$mean_elevation_m, wetlands$hans_m
...
m<-leaflet(wetlands) %>% 
  ...
  #Add subsheds
  addPolygons(data=proposed_catchments, weight =2, col="orange", group = "Proposed Catchments") %>% 
  #Add subsheds
...
saveWidget(m, file="site_selection_final.html")
